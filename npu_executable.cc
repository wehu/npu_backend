...
namespace npu {
    ...
    NpuExecutable::NpuExecutable(std::unique_ptr<xla::cpu::SimpleOrcJIT> jit,
    ...
    Status NpuExecutable::ExecuteThunks(
            const ServiceExecutableRunOptions* run_options,
            const NpuBufferAllocations& buffer_allocations, bool block_host_until_done,
        ...
        std::map<int32, const NpuThunk*> last_blocking_thunk_for_stream;
        std::map<const NpuThunk*, std::unique_ptr<se::Event>> thunk_to_finish_event;
        for (NpuThunk* thunk : thunk_schedule_->TotalOrder()) {
            ...
            for (const NpuThunk* dependency : thunk_schedule_->DependsOn(thunk)) {
    ...
    StatusOr<std::unique_ptr<ShapedBuffer>> NpuExecutable::ExecuteOnStream(
        ...
        NpuBufferAllocations::Builder buffer_allocations_builder;
    ...
    const PointsToSet& NpuExecutable::GetRootPointsToSet() const {
