...
// launch order.
        ...
        public:
            NpuHloOrdering(const HloModule *module,
                           const NpuStreamAssignment &stream_assignment,
                           const std::vector<const HloInstruction *> &thunk_launch_order);

            ~NpuHloOrdering() override = default;
...
// concurrently.
    ...
    NpuHloSchedule::NpuHloSchedule() {}

/* static */
    StatusOr<std::unique_ptr<NpuHloSchedule>> NpuHloSchedule::Build(
            const HloModule &module, const NpuStreamAssignment &stream_assignment,
            int64 pointer_size) {
        std::unique_ptr<NpuHloSchedule> schedule(new NpuHloSchedule);
        ...
        schedule->hlo_ordering_ = MakeUnique<NpuHloOrdering>(
